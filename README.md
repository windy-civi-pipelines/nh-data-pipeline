...
2. **Name your new repository** using the convention: `New Hampshire Data Pipeline` (e.g., `il-data-pipeline`, `tx-data-pipeline`).
...
3. **Update the state abbreviation** in both workflow files:
   ...
   **In `.github/workflows/scrape-and-format-data.yml`:**
   ...
   ```yaml
   env:
     STATE_CODE: nh # CHANGE THIS to your state abbreviation
   ...
   **In `.github/workflows/extract-text.yml`:**
   ...
   - name: Extract text
     ...
     with:
       state: nh # CHANGE THIS to your state abbreviation
   ```
...
uses: windy-civi/toolkit/actions/scrape@main
with:
  state: nh # State abbreviation (required)
...
git clone https://github.com/YOUR-ORG/New Hampshire Data Pipeline
cd New Hampshire Data Pipeline
